...
public class UserDAO {
    ...
    public boolean addUser(String username, String password, String email) {
        ...
        //      be sure to add username and hashed password to the document.
        // problem instructions will tell you the schema that the documents must follow.

        Map<String, Object> map = new HashMap<>();
        map.put("_id", username);
        map.put("password", passwordHash);

        Document user = new Document(map);
    ...
    public Document validateLogin(String username, String password) {
        ...
        MongoCursor<Document> userIterator = usersCollection.find(eq("_id", username)).iterator();

        // assign the result to the user variable.
        user = userIterator.tryNext();
    ...
    private String makePasswordHash(String password, String salt) {
        try {
            ...
            digest.update(saltedAndHashed.getBytes(StandardCharsets.UTF_8));
            byte hashedBytes[] = (new String(digest.digest(), "UTF-8")).getBytes(StandardCharsets.UTF_8);
